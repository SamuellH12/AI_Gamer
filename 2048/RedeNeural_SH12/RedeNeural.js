...
class RedeNeural {
    ...
    constructor( in_nodes, hide_nodes, hide_layers, out_nodes) {

        //NÚMERO DE NÓS
        this.in_nodes   = in_nodes;
        this.hide_nodes = hide_nodes;
        this.hide_layers = hide_layers;
        ...
        this.bias_hh = []
        for(let i=0; i < hide_layers - 1; i++)
        {
            this.bias_hh.push( new Matrix(this.hide_nodes, 1)  );  //bias entre as camadas de nós ocultos
        ...
        for(let i=0; i < hide_layers - 1; i++)
        {
            this.weigth_hh.push( new Matrix(this.hide_nodes, this.hide_nodes)  );  //bias entre as camadas de nós ocultos
            this.weigth_hh[i].randomize();           
        }
    ...
    predict( arr ){
        ...
        let ih = Matrix.multiply(this.weigth_ih, input );   //multiplica os PESOS entrada->oculta pela matriz do input
        ih = Matrix.add(ih, this.bias_ih);              //adiciona o bias com uma soma simples ao resultado da operação
        ih.map( signoid );                                //coloca o resultado na função de ativação, nesse caso a signoid


        let hh = ih;
        for(let i=0; i < this.hide_layers - 1; i++)
        {
            hh = Matrix.multiply(this.weigth_hh[i], hh);
            hh = Matrix.add(hh, this.bias_hh[i]);
            hh.map( signoid );
        }

        //processa os valores que vieram da camada OCULTA para o OUTPUT
        let ho = Matrix.multiply(this.weigth_ho, hh);   //multiplica os PESOS oculta->saída pela matriz que veio da oculta (que acabamos de processar)
        ho = Matrix.add(ho, this.bias_ho);              //adiciona o respectivo bias aqui também
        ho.map( signoid );                                    //pega essa matriz e passa ela pela função de ativação
        

        ho = Matrix.MatrixToArray(ho);  //transforma nosso output em um array
        return ho;                          //e devolve ele a quem o chamou
    }
    ...
    train(arr, target) {
        ...
        let ih = Matrix.multiply(this.weigth_ih, input );
        ih = Matrix.add(ih, this.bias_ih);  
        ih.map( signoid );
        ...
        //processa os valores que vieram da camada OCULTA para o OUTPUT
        let ho = Matrix.multiply(this.weigth_ho, hh);
        ho = Matrix.add(ho, this.bias_ho); 
        ho.map( signoid ); 
        ...
        let deriv_out = Matrix.map(output, deriv_signoid );  //cria uma derivada do nosso erro de saída, passando nosso valores pela derivada da função de ativação
        ...
        let deriv_hide = Matrix.map( hidden, deriv_signoid );            //cria uma derivada do erro da oculta com a função derivada sigmoid
